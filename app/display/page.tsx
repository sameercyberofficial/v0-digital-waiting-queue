...
import { Clock, Monitor, Users, Bell } from "lucide-react"
...
export default function PublicDisplay() {
  ...
  const startRealTimeUpdates = () => {
    intervalRef.current = setInterval(() => {
      fetchDisplayData()
    }, 2000) // Update every 2 seconds for public display
  }
  ...
  const fetchDisplayData = async () => {
    try {
      const [servingResponse, queueResponse] = await Promise.all([
        fetch("/api/display/now-serving"),
        fetch("/api/display/waiting-queue"),
      ])
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-blue-800 text-white p-8">
      ...
      <div className="text-center mb-12">
        <div className="flex items-center justify-center space-x-4 mb-6">
          <Monitor className="h-16 w-16 text-blue-200" />
        ...
        <div className="flex items-center justify-center space-x-8 text-xl">
          ...
          <div className="text-blue-200">Last Updated: {lastUpdate.toLocaleTimeString()}</div>
        </div>
      </div>
            ...
            {nowServing.length > 0 ? (
              nowServing.map((token, index) => (
                ...
                >
                  ...
                  <div className="text-xl text-blue-100">{token.service_name}</div>
                </div>
              ))
            ) : (
          ...
          <CardContent>
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {waitingQueue.slice(0, 8).map((token) => (
                ...
                >
                  <div className="flex items-center space-x-6">
                    <div className="text-4xl font-bold font-mono text-blue-200">{token.token_number}</div>
                    <div>
                      <div className="text-xl font-semibold">{token.service_name}</div>
                      <div className="text-lg text-blue-200">Position: {token.position_in_queue}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-semibold">~{token.estimated_wait_time}m</div>
                    <div className="text-sm text-blue-200">Est. Wait</div>
                  </div>
                </div>
              ))}
              ...
              {waitingQueue.length > 8 && (
                <div className="text-center py-4 text-xl text-blue-200">
      ...
      <div className="mt-12 grid md:grid-cols-4 gap-6">
        ...
        <Card className="bg-white/10 backdrop-blur-sm border-white/20">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold text-blue-300">{waitingQueue.length}</div>
